...
import {
  ...
  LogOut, Users, Copy, Check, Hash, Flame
} from 'lucide-react';
...
export default function Layout({ currentPage, onNavigate, children }: Props) {
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    if (error) {
      ...
      if (workspace) {
        ...
        const ws = workspaces.find(w => w.invite_code === joinCode.toUpperCase());
  ...
  return (
    <div className="flex h-screen bg-[#020817] overflow-hidden">
      {/* Sidebar */}
      <aside className="w-64 flex-shrink-0 bg-white/[0.02] border-r border-white/[0.06] flex flex-col">
        {/* Logo */}
        ...
        <div className="p-3 border-b border-white/[0.06]">
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/[0.05] transition-colors group"
          >
            {activeWorkspace ? (
              <>
                <div className="w-6 h-6 rounded bg-gradient-to-br from-blue-600 to-cyan-500 flex items-center justify-center flex-shrink-0">
                ...
                {workspaces.map(ws => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-2.5 hover:bg-white/[0.05] transition-colors text-left ${
                      activeWorkspace?.id === ws.id ? 'bg-blue-600/10' : ''
                    }`}
                  >
                    <div className="w-6 h-6 rounded bg-gradient-to-br from-blue-600 to-cyan-500 flex items-center justify-center flex-shrink-0">
                      <Hash size={11} className="text-white" />
        ...
        {/* Invite code */}
        {activeWorkspace && (
          <div className="mx-3 mb-2 p-3 bg-white/[0.03] rounded-xl border border-white/[0.06]">
        ...
        {activeWorkspace && activeWorkspace.streak_days > 0 && (
          <div className="mx-3 mb-2 flex items-center gap-2 px-3 py-2 bg-orange-500/10 border border-orange-500/20 rounded-xl">
        ...
        <div className="p-3 border-t border-white/[0.06]">
          <div className="flex items-center gap-3">
            <img
              src={
                profile?.avatar_url ||
                `https://api.dicebear.com/7.x/initials/svg?seed=${profile?.full_name || 'User'}`
              }
              alt="avatar"

...
export default function AnalyticsPage() {
  ...
  const totalEstimatedDays = tasks.reduce(
    (sum, task) => sum + (task.estimated_days || 0),
    0
  );
  const completedEstimatedDays = tasks
    .filter((t) => t.status === 'completed')
    .reduce((sum, task) => sum + (task.estimated_days || 0), 0);
    ...
    return members
      .map((m, idx) => {
        ...
        const contributionScore = Math.round(
          taskScore + uploadScore + activityScore + speedScore
        );
  ...
  const fastestContributor = [...memberStats].sort(
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-white font-medium truncate">
                    {m.name}
                    {m.isCurrentUser && <span className="text-blue-400 ml-1">(you)</span>}
                  </span>
                  <span className="text-xs font-bold ml-2" style={{ color: m.color }}>
                    {m.contributionScore}%
                  </span>
                </div>
                ...
                <div className="h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${m.contributionScore}%` }}
                  ...
                  <div key={row.userId}>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-[10px] text-slate-500">
                        {row.rows.flat().reduce((sum, cell) => sum + cell.count, 0)}
                      </span>
                    </div>
                    <div className="grid grid-cols-7 gap-0.5">
                      {row.rows.flat().map((cell) => (
                        <div
                          key={`${row.userId}-${cell.date.toISOString()}`}
                          className={`h-5 rounded-sm ${cell.value}`}
                          title={`${cell.count} completed on ${cell.date.toLocaleDateString()}`}
                        />
                      ))}
                    </div>
                  </div>
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-white truncate">
                    {m.name}
                    {m.isCurrentUser && <span className="text-blue-400 text-xs ml-1">(you)</span>}
                  </p>
